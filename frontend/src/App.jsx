    ...
    if (activeMode) {
        return (
            <div className="min-h-screen bg-gray-50">
                <Header username={username} onLogout={onLogout} onBack={() => {setActiveMode(null); setResult(null);}} />
                <div className="container mx-auto px-4 py-8">
                    {activeMode === 'chat' && (
                        <ChatInterface onResult={setResult} setLoading={setLoading} loading={loading} />
                    )}
                    {activeMode === 'image' && (
                        <ImageInterface onResult={setResult} setLoading={setLoading} loading={loading} />
                    )}
                    {activeMode === 'voice' && (
                        <VoiceInterface onResult={setResult} setLoading={setLoading} loading={loading} />
                    )}
            ...
            <div className="grid md:grid-cols-2 gap-6">
                {result.weather_data && (
                    <div className="bg-blue-50 border border-blue-200 p-6 rounded-lg">
                        ...
                        <div className="space-y-2 text-sm">
                            <p><span className="font-medium">Temperature:</span> {result.weather_data.temperature}°C</p>
                            <p><span className="font-medium">Humidity:</span> {result.weather_data.humidity}%</p>
                            <p><span className="font-medium">Condition:</span> {result.weather_data.description}</p>
                            <p><span className="font-medium">Wind Speed:</span> {result.weather_data.wind_speed} m/s</p>
                        </div>
                    </div>
                )}
                ...
                {result.market_data && (
                    <div className="bg-green-50 border border-green-200 p-6 rounded-lg">
                        ...
                        <div className="space-y-2 text-sm">
                            <p><span className="font-medium">Crop:</span> {result.market_data.crop}</p>
                            <p><span className="font-medium">Price:</span> ₹{result.market_data.price_per_quintal}/quintal</p>
                            <p><span className="font-medium">Market:</span> {result.market_data.market}</p>
                            <p><span className="font-medium">Trend:</span> {result.market_data.trend}</p>
                        </div>
                    </div>
                )}
            </div>
